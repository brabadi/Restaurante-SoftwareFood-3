...
// Fin lista Clientes
    ...
    /**
     * Envia la solicitud al servidor para guardar un Pedido
     *
     * @param instance la instancia a guardar
     * @return identificador del pedido el cual se ha hecho la solicutd de guardado
     * @throws Exception
     */

    @Override
    public String savePedido(Pedido instance) throws Exception {
        //devuelve un string en formato Json que lo que se enviara
        String requestJson = createPedidoJson(instance);
        if ((this.processConnection(requestJson)).equals("FALLO")) {
            return null;
        }
        return String.valueOf(instance.getPed_Id());
    }
    
    /**
     * Crea el Pedido recibido en un json para el envio por el sockect al
     * servidor
     * @param instance pedido que se llevera formato json
     * @return
     */
    private String createPedidoJson(Pedido instance) {
        Protocol protocol = new Protocol();
        //el orden debe ser respetado
        protocol.setResource("cliente");
        protocol.setAction("guardarPedido");
        
        protocol.addParameter("ped_Id", String.valueOf(instance.getPed_Id()));
        protocol.addParameter("cli_Id", String.valueOf(instance.getCli_Id()));
        protocol.addParameter("res_Nit", String.valueOf(instance.getRes_Nit()));
        protocol.addParameter("plaEje_Dia", String.valueOf(instance.getPlaEje_Dia().getIDplatoEJ()));
        protocol.addParameter("ped_Cantidad", String.valueOf(instance.getPed_Cantidad()));
        protocol.addParameter("ped_fecha", String.valueOf(instance.getPed_fecha()));
        protocol.addParameter("ped_Hora", instance.getPed_Hora());
        protocol.addParameter("ped_Estado", String.valueOf(instance.getPed_Estado()));
        protocol.addParameter("ped_PrecioTotal", String.valueOf(instance.getPed_PrecioTotal()));
   ...
   /**
     * Realiza el proceso de conexion entre el cliente y el servidor, y
     * deacuerdo su estado se obtendra el exito o fallo de la solicitud de
     * obtencion de los pedido.
     *
     * @return lista de tipo Pedido
     * @throws Exception
     */ 
    @Override
    public List<Pedido> listarPedido() throws Exception {
        String resource = "administrador";
        String accion = "listarPedido";
        String requestJson = createlistPedido(resource, accion);
        String response = procesarConexion(requestJson);
        return jsonListarPedido(response);
    }
     ...
     * @return
     */
    private String createlistPedido(String resource, String accion) {
        Protocol protocol = new Protocol();
        protocol.setResource(resource);
        protocol.setAction(accion);
    ...
    /**
     * Obtiene la lista de tipo Pedido resiviendo el estado de la
     * conexion para su posteror envio al servidor
     *
     * @param jsonListarPedido la accion que se esta solicitando
     * @return lista de tipo Pedido.
     */
    private List<Pedido> jsonListarPedido(String jsonListarPedido) {
        Gson gson = new Gson();
        Type list = new TypeToken<List<Pedido>>() {
        }.getType();
        return gson.fromJson(jsonListarPedido, list);
    }
